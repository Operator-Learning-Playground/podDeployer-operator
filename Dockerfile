...
COPY pkg/ pkg/
# Build
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -o mypoddeployeroperator main.go


FROM alpine:3.12
WORKDIR /app
# 需要的文件需要复制过来
COPY --from=builder /app/mypoddeployeroperator .
USER 65532:65532

ENTRYPOINT ["./mypoddeployeroperator"]